...
RSpec.describe 'Oauth user verification' do
  context 'registering' do
    it 'returns CLEAR if an oauth email does not exist in the db' do
      VCR.use_cassette('oauth-register-path') do
        user_params =  "jackjohnson@faker.net"
    ...
    context 'happy path' do
      it 'logs an oauth user in if they exist' do
        VCR.use_cassette('oauth-login-path') do
          ...
          user_data = "bobby@faker.net"

          headers = { "CONTENT_TYPE" => "application/json" }
          post "/api/v1/sessions/auth", headers: headers, params: user_data, as: :json
          ...
          expect(attributes).to have_key(:oauth)
          expect(attributes[:oauth]).to eq(true)
        end
      end
    end
  end
