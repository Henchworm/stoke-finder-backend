...
RSpec.describe 'Creating a user' do
  context 'non-OAuth user' do
    ...
    it "edge case: access passed as string", :vcr do
      user_params =  {
      user_name: "BobBarker123",
      email: "email@faker.net",
      password: "Guest",
      ...
      expect(return_value[:message]).to eq("Access variable needs to be an integer")
    end
    ...
    it "edge case: activity_preferences passed as string", :vcr do
      ...
      expect(return_value[:message]).to eq("Activity_preferences needs to be an array")
    end
  end
end
